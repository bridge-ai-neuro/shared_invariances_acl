...
class GreedyWordSwapWIR(SearchMethod):
    ...
    def perform_search(self, initial_result):
        ...
        while i < len(index_order) and not search_over:
            ...
            results = sorted(new_results, key=lambda x: -x.score)
            # Skip swaps which don't improve the score

            if results[0].score > cur_result.score:
                cur_result = results[0] 
                # print(f'Initial Text: {initial_result.attacked_text.text} | Attacked Text: {cur_result.attacked_text.text} | Goal-Status: {cur_result.goal_status}')
            else:
                continue
        ...
        if cur_result.attacked_text.text == initial_result.attacked_text.text:
            scores_ = np.array([r.score for r in results])
            failed_best_result = results[scores_.argmax()]
            cur_result = failed_best_result
